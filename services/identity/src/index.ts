...
app.get("/health", async (request, reply) => {
  ...
  return reply.send(ok({ status: "ok", service: "identity-service" }, requestId));
});
...
app.get("/health/ready", async (request, reply) => {
  ...
  return reply.send(ok({ status: "ready", service: "identity-service" }, requestId));
});
...
app.get("/v1/auth/status", async (request, reply) => {
  ...
  return reply.send(
    ok(
      {
        provider: "supabase",
        note: "POC uses service-role DB access; JWT auth wired in Phase 1",
      },
      requestId
    )
  );
