-- ConveyX Order + Billing — run this ENTIRE file in Supabase SQL Editor
-- Creates schemas/tables (if missing) then RPC functions.
...
DECLARE v_order cx_order.orders;
  v_lines json;
BEGIN
...
DECLARE v_order cx_order.orders;
  ...
  v_sort int := 0;
BEGIN
  INSERT INTO cx_order.orders (
    entity_id, property_address, property_context, include_body_corp, status
  ) VALUES (
    (p_data->>'entity_id')::uuid,
    p_data->>'property_address',
  ...
  LOOP
    ...
    INSERT INTO cx_order.order_lines (
      ...
      coalesce(v_line->'required_data_buyer', '{}'),
      coalesce(v_line->'required_data_seller', '{}'),
      v_sort
    );
    v_sort := v_sort + 1;
  END LOOP;
...
BEGIN
  ...
  IF v_order.status != 'draft' THEN
    RAISE EXCEPTION 'Order cannot be submitted from status %', v_order.status;
  END IF;
  ...
  INSERT INTO billing.transactions (
    order_id, entity_id, reference, description, amount, gst_amount, total, status
  ) VALUES (
    p_id, v_order.entity_id, v_ref,
    'Order ' || v_ref || ' — ' || v_order.property_address,
...
DECLARE v_inv billing.invoices;
  v_lines json;
BEGIN
...
DECLARE v_inv billing.invoices;
  ...
  v_gst numeric := 0;
  v_total numeric := 0;
  v_tx billing.transactions;
BEGIN
  v_num := 'INV-' || to_char(now(), 'YYYYMM') || '-' || nextval('billing.invoice_number_seq');

  INSERT INTO billing.invoices (entity_id, invoice_number, status)
  VALUES (p_entity_id, v_num, 'issued')
  RETURNING * INTO v_inv;

  FOR v_tx IN
    SELECT * FROM billing.transactions
    WHERE entity_id = p_entity_id AND status = 'pending'
      AND (p_transaction_ids IS NULL OR id = ANY(p_transaction_ids))
    FOR UPDATE
  LOOP
    INSERT INTO billing.invoice_lines (
      invoice_id, transaction_id, order_id, description, amount, gst_amount, line_total
    ) VALUES (
      v_inv.id, v_tx.id, v_tx.order_id, v_tx.description,
      v_tx.amount, v_tx.gst_amount, v_tx.total
    );
    v_subtotal := v_subtotal + v_tx.amount;
    v_gst := v_gst + v_tx.gst_amount;
    v_total := v_total + v_tx.total;

    UPDATE billing.transactions SET status = 'invoiced' WHERE id = v_tx.id;
  END LOOP;

  IF v_total = 0 THEN
    DELETE FROM billing.invoices WHERE id = v_inv.id;
    RAISE EXCEPTION 'No pending transactions to invoice';
  END IF;

  UPDATE billing.invoices SET
    subtotal = v_subtotal, gst_total = v_gst, total = v_total, issued_at = NOW()
